...
function theme_mb2nl_login_form ()
{
	...
	if ((!isloggedin() || isguestuser()) && !$link_to_login)
	{
		...
		$output .= '<p class="login-info"><a href="https://kelaspersonalia.id/login/signup.php?">Klik Disini Untuk Daftar</a></>'
		;
...
function theme_mb2nl_header_tools ()
{
	...
	if (!$isLoginPage)
	{
		...
		if ((!isloggedin()) && $link_to_login) // || isguestuser()
        {
			$jslink_cls = '';
           ...
           else
           {
            ...
            $output .= '<a href="' . $login_link . '" class="header-tools-link' . $jslink_cls . ' tool-login" >';
            ...
            $output .= '<i class="icon1 fa fa-' . $loginicon . '"></i>';
            //$output .= $login_text;
            $output .= ' Masuk | Daftar </a>';
            // $output .= '<a href="https://kelaspersonalia.id/login/signup.php" class="header-tools-link' . $jslink_cls . ' tool-login" >';
            //$output .= '<i class="icon1 fa fa-' . $loginicon . '"></i>';
            //$output .= $login_text;
            // $output .= ' Daftar </a>';
        }elseif(isloggedin()){
